...
void displaySystemMSG(bool MQTT_connection_state, uint8_t displayMode);
...
uint32_t readSensTime , setScreenTime, sendTime, setMSGTime;
...
void setup() 
{
  ...
  setMSGTime = millis() + setMSGInterval;
}
...
//      Serial.println("error to read Temp and Humid")
  ...
  if (millis() >= setMSGTime){
    displaySystemMSG(true, displayMSGmode);
    displayMSGmode++;
    if (displayMSGmode >= 3)
      displayMSGmode = 0;
    while (millis() >= setMSGTime)
      setMSGTime += setMSGInterval;
  }
...
void displaySystemMSG(bool MQTT_connection_state, uint8_t displayMode){
  ...
  if (displayMode == 0){
    ...
    if (WiFi.status() == WL_CONNECTED)  {tft.setTextColor(TFT_DARKGREEN, TFT_LIGHTGREY); tft.drawString("                          ", 360, 305, GFXFF);   tft.drawString(WiFi.SSID(), 360, 305, GFXFF);}
    else                                {tft.setTextColor(TFT_RED,       TFT_LIGHTGREY); tft.drawString("LOSS                      ", 360, 305, GFXFF);   Serial.println("***LOSS Wi-Fi***");}
  }
    ...
    if (MQTT_connection_state == true)  {tft.setTextColor(TFT_DARKGREEN, TFT_LIGHTGREY); tft.drawString("CONNECTED                 ", 367, 305, GFXFF);}
    else                                {tft.setTextColor(TFT_RED,       TFT_LIGHTGREY); tft.drawString("LOSS                      ", 367, 305, GFXFF);}
  }
}
